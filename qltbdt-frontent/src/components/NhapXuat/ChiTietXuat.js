...
// import { getPhieuXuatByIdAPI } from '../../api'; 
// import { formatDate, formatCurrency } from '../../utils/helpers';
// import { getTinhTrangLabel } from '../../utils/constants';

// const ChiTietXuat = ({ phieuXuatId, onClose }) => { 

//     const { data: phieuXuat, isLoading, error } = useQuery({
//         queryKey: ['phieuXuatDetail', phieuXuatId],
//         queryFn: () => getPhieuXuatByIdAPI(phieuXuatId).then(res => res.data),
//         enabled: !!phieuXuatId, 
//     });
...
//     if (!phieuXuat) return <p className="p-4 text-gray-500">Không tìm thấy chi tiết phiếu xuất.</p>;
...
//              <div>
//                 <h4 className="mb-2 text-base font-semibold text-purple-700">Danh sách tài sản đã xuất ({phieuXuat.chiTietThietBi?.length || 0})</h4>
//                 {phieuXuat.chiTietThietBi && phieuXuat.chiTietThietBi.length > 0 ? (
...
//                         {phieuXuat.chiTietThietBi.map(item => (
//                             <li key={item.thongtinthietbi_id}>
//                                 ID: {item.thongtinthietbi_id} - {item.tenThietBi}
//                              </li>
//                         ))}
//                     </ul>
//                 ) : (
