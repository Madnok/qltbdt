...
import React, { useState } from 'react';
import { useMutation, useQueryClient } from '@tanstack/react-query'; // Import lại useQueryClient
import { FaTimesCircle } from 'react-icons/fa';
// Chỉ cần import API tạo log và upload ảnh
import { createLogBaoTriAPI, uploadInvoiceImagesAPI as uploadImagesAPI } from '../../api';
import { toast } from 'react-toastify';
import { ArrowPathIcon } from '@heroicons/react/24/outline';

const FormNhanBaoHanhVe = ({ deviceInfo, onClose, onSuccess }) => {
    const [ketQuaNhanHang, setKetQuaNhanHang] = useState('tot'); // Mặc định là tốt
    const [hoatdong, setHoatdong] = useState(
        `Nhận thiết bị ${deviceInfo?.tenThietBi || `ID ${deviceInfo?.id}` || 'N/A'} từ bảo hành về.` +
        (deviceInfo?.relatedBaoHongId ? ` (Báo hỏng gốc ID: ${deviceInfo.relatedBaoHongId})` : '')
    );
    ...
    const [damagePreviews, setDamagePreviews] = useState([]);
    const [error, setError] = useState('');
    const queryClient = useQueryClient(); // Lấy queryClient để invalidate

    // --- Chỉ cần Mutation tạo log ---
    const createLogMutation = useMutation({
        mutationFn: createLogBaoTriAPI,
        onSuccess: (data) => {
            // Xử lý thành công chung
            toast.success(data.message || `Đã ghi nhận nhận thiết bị ${deviceInfo?.id}. Trạng thái báo hỏng đã được cập nhật.`);
            // Invalidate các query liên quan để cập nhật dữ liệu mới
            queryClient.invalidateQueries({ queryKey: ['baoHongList'] }); // Cập nhật danh sách admin
            queryClient.invalidateQueries({ queryKey: ['myTasks'] });     // Cập nhật task của NV (nếu cần)
            queryClient.invalidateQueries({ queryKey: ['lichSuThietBi', deviceInfo?.id] }); // Cập nhật lịch sử của thiết bị này
            queryClient.invalidateQueries({ queryKey: ['taiSanList'] }); // Cập nhật danh sách tài sản nếu trạng thái thay đổi
            if (typeof onSuccess === 'function') { onSuccess(); }
            onClose(); // Đóng modal
        },
        onError: (err) => {
            // Xử lý lỗi chung
            console.error("!!! Lỗi khi tạo log nhận hàng bảo hành:", err.response?.data || err.message || err);
            let errorMsg = 'Có lỗi xảy ra trong quá trình xử lý.';
            if (err.response?.data?.error) {
                errorMsg = err.response.data.error;
            } else if (err.response?.data?.message) {
            ...
            setError(`Lỗi: ${errorMsg}`); // Hiển thị lỗi trên form
            toast.error(`Thất bại: ${errorMsg}`); // Thông báo toast lỗi
        }
    });

    // Bỏ các mutation thừa:
    ...
    const handleSubmit = async (e) => {
        ...
        // --- Validation ---
        if (!deviceInfo?.relatedBaoHongId || !deviceInfo?.id) {
            setError('Lỗi: Thiếu thông tin ID báo hỏng gốc hoặc ID thiết bị.');
            return;
        }
        if (!hoatdong.trim() || !ketQuaNhanHang) {
            setError('Vui lòng nhập hoạt động và chọn kết quả nhận hàng.');
        ...
        try {
            // Chỉ cần gọi mutation tạo log, không cần quản lý loading riêng lẻ
            // const isProcessing = createLogMutation.isPending; // Dùng isPending của mutation

            // --- 1. Upload ảnh (Nếu có) ---
            if (damageFiles.length > 0) {
                console.log("[FormNhanVe] Uploading damage images...");
                // **LƯU Ý:** Giả sử uploadImagesAPI trả về Promise chứa mảng URL
                // Cần xử lý loading/error cho bước này nếu muốn chi tiết hơn
                try {
                    uploadedDamageUrls = await uploadImagesAPI(damageFiles); // Đảm bảo API này đúng
                    console.log("[FormNhanVe] Damage images uploaded:", uploadedDamageUrls);
                } catch (uploadError) {
                    console.error("[FormNhanVe] Lỗi upload ảnh:", uploadError);
                    throw new Error("Lỗi khi tải ảnh lên. Vui lòng thử lại."); // Ném lỗi để dừng và báo user
                }
            }

            // --- 2. Chuẩn bị và Tạo Log ---
            const ketQuaXuLyLog = 'Đã nhận từ bảo hành';
            let hoatdongText = hoatdong.trim();
            // Bổ sung chi tiết vào hoạt động
            if (ketQuaNhanHang === 'con_loi') hoatdongText += ' - Tình trạng ghi nhận: Vẫn còn lỗi.';
            else if (ketQuaNhanHang === 'gui_lai') hoatdongText += ' - Hành động tiếp theo: Gửi lại bảo hành.';
            else if (ketQuaNhanHang === 'thanh_ly') hoatdongText += ' - Hành động tiếp theo: Đề xuất thanh lý.';
            else hoatdongText += ' - Tình trạng ghi nhận: Hoạt động tốt.';
            ...
            const logData = {
                baohong_id: deviceInfo.relatedBaoHongId,
                thongtinthietbi_id: deviceInfo.id,
                phong_id: deviceInfo.phong_id || null, // Lấy ID phòng từ deviceInfo
                hoatdong: hoatdongText,
                ketQuaXuLy: ketQuaXuLyLog,          // Luôn là 'Đã nhận từ bảo hành'
                phuongAnXuLy: 'Bảo hành',            // Có thể giữ là 'Bảo hành'
                phuongAnKhacChiTiet: null,           // Không cần thiết khi phương án là BH
                suDungVatTu: false,                  // Mặc định không khi nhận về
                hinhAnhHongHocUrls: uploadedDamageUrls.length > 0 ? uploadedDamageUrls : null, // Ảnh lúc nhận về
                // Các trường khác để null
                ghiChuVatTu: null,
                chiPhi: null,
                hinhAnhHoaDonUrls: null,
                ngayDuKienTra: null,
                // Backend sẽ tự thêm nhanvien_id và thoiGian
                // Thêm cả 'ketQuaNhanHang' để backend biết xử lý trạng thái TTTB/BH
                // Mặc dù backend mới đã dựa vào ketQuaXuLy, gửi thêm cũng không sao
                // Hoặc không cần gửi nếu backend chỉ dựa vào 'Đã nhận từ bảo hành'
            };

            console.log("[FormNhanVe] Calling createLogMutation with data:", logData);
            // Gọi mutation tạo log, không cần await ở đây vì đã có onSuccess/onError
            createLogMutation.mutate(logData);

            // --- KHÔNG CẦN CÁC BƯỚC CẬP NHẬT THỪA NỮA ---
            // Bỏ: await updateDeviceStatusMutation.mutateAsync(...)
            // Bỏ: await updateBaoHongStatusMutation.mutateAsync(...)

        } catch (err) {
            // Lỗi ở bước upload ảnh hoặc lỗi đồng bộ khác trước khi gọi mutate
            console.error("[FormNhanVe] Lỗi trong handleSubmit (trước khi gọi mutate):", err);
            const errorMsg = err.message || "Có lỗi xảy ra.";
            setError(`Lỗi: ${errorMsg}`);
            toast.error(`Thất bại: ${errorMsg}`);
        }
        // Không cần finally ở đây vì loading dựa vào mutation.isPending
    };

    // Lấy trạng thái loading từ mutation
    const isProcessing = createLogMutation.isPending;
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black bg-opacity-50">
            <div className="w-full max-w-lg p-6 bg-white rounded-lg shadow-xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center justify-between pb-3 mb-4 border-b">
                    <h2 className="text-xl font-semibold">Nhận Thiết Bị Bảo Hành Về</h2>
                    <button onClick={onClose} className="text-gray-500 hover:text-gray-700" disabled={isProcessing}> <FaTimesCircle size={24} /> </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        <select id="ketqua-nhanhang" value={ketQuaNhanHang} onChange={(e) => setKetQuaNhanHang(e.target.value)} className="w-full p-2 bg-white border rounded-md" required disabled={isProcessing}>
                            <option value="tot">Hoạt động tốt</option>
                            <option value="con_loi">Vẫn còn lỗi</option>
                            {/* Các option này có thể không cần thiết nếu backend tự xử lý trạng thái TTTB/BH */}
                            {/* <option value="gui_lai">Gửi lại bảo hành</option> */}
                            {/* <option value="thanh_ly">Đề xuất thanh lý</option> */}
                        </select>
                        {/* Bỏ dòng giải thích này nếu không cần */}
                        {/* <p className="mt-1 text-xs text-gray-500">Trạng thái thiết bị và báo hỏng sẽ được cập nhật tương ứng.</p> */}
                    </div>

                    {/* Ảnh tình trạng lúc nhận */}
                    <div>
                        <label htmlFor="damage-upload-nhanhang" className="block mb-1 text-sm font-medium text-gray-700">Ảnh tình trạng thiết bị khi nhận (Tùy chọn, tối đa 2 ảnh):</label>
                        <input type="file" id="damage-upload-nhanhang" multiple accept="image/*"
                            onChange={handleFileChange}
                            className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-gray-50 file:text-gray-700 hover:file:bg-gray-100"
                            disabled={isProcessing || damageFiles.length >= 2} />
                        {/* Preview ảnh */}
                        <div className="flex flex-wrap gap-2 mt-2">
                            {damagePreviews.map((preview, index) => (
                                <div key={index} className="relative">
