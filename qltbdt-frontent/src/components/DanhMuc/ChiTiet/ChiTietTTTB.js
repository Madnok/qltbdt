...
const ChiTietTTTB = ({ onClose, record, refreshData }) => {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_API_URL}/tttb/thietbi-list`, {withCredentials:true})
    ...
    const handleSave = async () => {
        try {
            await axios.put(`${process.env.REACT_APP_API_URL}/tttb/${record.id}`, editData, { withCredentials: true });
    ...
    //         try {
    //             await axios.delete(`${process.env.REACT_APP_API_URL}/tttb/${record.id}`, { withCredentials: true });
