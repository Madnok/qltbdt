...
const ConfirmCompleteModal = ({ isOpen, onClose, data, onSubmit, isLoading }) => {
    // State local để lưu trạng thái TB cuối cùng được chọn trong modal (nếu có)
    const [selectedFinalStatus, setSelectedFinalStatus] = useState('');
    const [error, setError] = useState('');

    // Xác định xem có cần hiển thị dropdown không và các options là gì
    const needsDeviceStatusSelection = data.originalDeviceStatus === 'da_bao_hanh' || data.originalDeviceStatus === 'de_xuat_thanh_ly';
    let dropdownOptions = [];
    let dropdownLabel = '';

    if (data.originalDeviceStatus === 'da_bao_hanh') {
        dropdownLabel = 'Trạng thái thiết bị sau bảo hành';
        dropdownOptions = [
            { value: '', label: '-- Chọn trạng thái --' },
            { value: 'con_bao_hanh', label: 'Còn Bảo Hành' },
            { value: 'het_bao_hanh', label: 'Hết Bảo Hành' },
        ];
    } else if (data.originalDeviceStatus === 'de_xuat_thanh_ly') {
        dropdownLabel = 'Xử lý đề xuất thanh lý';
        dropdownOptions = [
            { value: '', label: '-- Chọn hành động --' },
            { value: 'cho_thanh_ly', label: 'Chờ Thanh Lý (Duyệt)' },
            { value: 'con_bao_hanh', label: 'Còn Bảo Hành (Từ chối)' },
            { value: 'het_bao_hanh', label: 'Hết Bảo Hành (Từ chối)' },
        ];
    }

    // Reset state local khi modal mở
    useEffect(() => {
        if (isOpen) {
    ...
    const handleSubmit = () => {
        ...
        setError('');
        // Gọi hàm submit từ props, truyền cả finalStatus đã chọn (hoặc null nếu không cần)
        onSubmit(data.baoHongId, selectedFinalStatus || null);
    };
    ...
    return (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black bg-opacity-60">
            <div className="w-full max-w-md p-6 bg-white rounded-lg shadow-xl">
                <h3 className="mb-4 text-lg font-semibold text-green-700">Xác nhận Hoàn thành Công việc</h3>
                <p className="mb-3 text-sm">Bạn có chắc muốn đóng công việc cho báo hỏng ID: <span className='font-bold'>{data.baoHongId}</span>?</p>

                {/* Dropdown chọn trạng thái cuối (hiển thị có điều kiện) */}
                {needsDeviceStatusSelection && (
                    <div className="mb-4">
                        <label htmlFor="finalDeviceStatusSelect" className="block mb-1 text-sm font-medium text-gray-700">
                        ...
                        <select
                            ...
                            className={`w-full px-2 py-1.5 text-sm border rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 ${error ? 'border-red-500' : 'border-gray-300'}`}
                        >
                ...
                <div className="flex justify-end gap-3 mt-6">
                    <button type="button" onClick={onClose} disabled={isLoading} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50">Hủy bỏ</button>
                    <button
                        onClick={handleSubmit}
                        // Disable nếu đang loading HOẶC nếu cần chọn mà chưa chọn
...
//                                                             <p className="mt-1">Tình Trạng: <span className="inline-flex items-center text-purple-600">{getTinhTrangLabel(item.tinhTrang)}</span></p>
//                                                             <p className="mt-1"><strong>Mô Tả:</strong> {item.moTa || "Không có mô tả"}</p>
//                                                             <p className="mt-1"><strong>Hình Ảnh:</strong></p>
//                                                             {item.hinhAnh ? (
